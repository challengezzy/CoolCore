...
public class DatabaseAdapter implements DatabaseInfo {
    ...
    public static DatabaseAdapter getInstance(String dsClassName) throws Exception {
        ...
        if (dsClassName.indexOf("sybase") != -1) {
            //self.setDB(new SybaseDatabaseInfo());
        } else if (dsClassName.indexOf("oracle") != -1) {
    ...
    public static DatabaseAdapter getInstance(int dbType) throws Exception {
        ...
        self = new DatabaseAdapter();
        if (dbType == DBInfoConst.DATABASE_TYPE_SYBASE) {
            //self.setDB(new SybaseDatabaseInfo());
        } else if (dbType == DBInfoConst.DATABASE_TYPE_ORACLE) {
            self.setDB(new OracleDatabaseInfo());
        } else {
    ...
    public Vector getErrDesc(SQLException dbException, DBConnection session) {
